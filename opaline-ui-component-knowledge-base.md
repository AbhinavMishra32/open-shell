# Opaline UI Component System Knowledge Base

This document is the long-term reference for recreating the Opaline Electron UI in another Electron app with the same visual system and the same component boundaries.

Use this alongside:
- [opaline-ui-inventory.md](/Users/abhinavmishra/solin/general/desktop-agent-app/opaline-ui-inventory.md)
- [opaline-same-ui-electron](/Users/abhinavmishra/solin/general/desktop-agent-app/opaline-same-ui-electron)

## 1. Goal

Build a second Electron app whose UI matches the Opaline desktop app as closely as possible, using the same component system, same shell structure, same spacing and motion rules, and ideally the same source code shape where that is practical and permitted.

## 2. What This UI Really Is

The Opaline desktop UI is not a single screen.
...
It has two jobs:
- run the original Opaline Electron main process for exact behavioral/log comparison
...
- `opaline-same-ui-electron/src/component-library`

Component-library preview renderer path:
- `opaline-same-ui-electron/src/renderer`
...
- `src/lib/opaline-ui` is the readable stripped shared UI library used by the non-agent copy app.
- New apps that want the same Opaline UI should render through `src/lib/opaline-ui` and use `src/component-library` as the literal upstream source/reference catalog.
...
- `opaline-same-ui-electron/src/lib/opaline-ui/app-shell/AppShell.tsx`
- `opaline-same-ui-electron/src/lib/opaline-ui/app-shell/app-shell.css`
...
- `opaline-same-ui-electron/src/component-library/shell/index.js`
- `opaline-same-ui-electron/src/component-library/original/assets/app-shell-D7yvB1FT.js`
- `opaline-same-ui-electron/src/component-library/original/assets/app-shell-DJDX7Pvr.css`
...
Reconstructed source now started at:
- `opaline-same-ui-electron/src/lib/opaline-ui/tokens/opaline-theme.css`
- `opaline-same-ui-electron/src/lib/opaline-ui/primitives/Button.tsx`
- `opaline-same-ui-electron/src/lib/opaline-ui/primitives/Dialog.tsx`
- `opaline-same-ui-electron/src/lib/opaline-ui/bottom-panel/BottomPanel.tsx`
- `opaline-same-ui-electron/src/lib/opaline-ui/file-tree/FileTree.tsx`
- `opaline-same-ui-electron/src/lib/opaline-ui/icons/OpalineMark.tsx`
- `opaline-same-ui-electron/src/lib/opaline-ui/sidebar/Sidebar.tsx`
- `opaline-same-ui-electron/src/lib/opaline-ui/thread/ThreadSurface.tsx`
- `opaline-same-ui-electron/src/lib/opaline-ui/composer/Composer.tsx`

Literal upstream shared component extraction now lives at:
- `opaline-same-ui-electron/src/component-library/styles/index.js`
- `opaline-same-ui-electron/src/component-library/primitives/index.js`
- `opaline-same-ui-electron/src/component-library/primitives/button.js`
- `opaline-same-ui-electron/src/component-library/composer/index.js`
- `opaline-same-ui-electron/src/component-library/thread/index.js`
- `opaline-same-ui-electron/src/component-library/sidebar/index.js`
...
- `src/renderer/App.tsx` imports copied upstream styles and the readable stripped UI library from `src/lib/opaline-ui`.
...
- Use `src/component-library` as source/reference data, then expose compatible readable components through `src/lib/opaline-ui`.
...
Opaline builds many shared primitives over Radix-style building blocks.
...
- The readable library now includes `src/lib/opaline-ui/primitives/DropdownMenu.tsx` as a Radix-backed wrapper and `Button.tsx` supports Radix `Slot` via `asChild`.
...
- Preserve Opaline class contracts and CSS tokens while replacing handwritten behavior with Radix behavior.

Current readable Radix-backed ports:
- `Button.tsx`: supports Radix `Slot` via `asChild`.
- `DropdownMenu.tsx`: wraps `@radix-ui/react-dropdown-menu` and keeps Opaline dropdown CSS contracts.
- `Dialog.tsx`: wraps `@radix-ui/react-dialog` and ports the Opaline dialog wrapper from `dialog-layout-CCvvb1Vc.js`.
...
- Overlay class is `opaline-dialog-overlay` with Electron overlay background `#00000022`.
- Content base class is `opaline-dialog left-1/2 top-1/2 z-50 -translate-x-1/2 -translate-y-1/2 outline-none`.
...
Current readable ports:
- `packages/ui/src/app-shell/AppShell.tsx` renders a Opaline-shaped active tab controller plus adjacent `Chat actions` ellipsis dropdown.
...
Renderer evidence:
- `app-main-C3VNTc8v.js` sets `document.documentElement.dataset.opalineWindowType = "electron"` and `dataset.opalineOs`.
