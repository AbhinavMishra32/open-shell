...
| Open Shell package | App shell and topbar | `packages/ui/src/app-shell/AppShell.tsx` | high | Readable package port of app-shell tab controller/header slots from `app-shell-D7yvB1FT.js`; active tab now owns the thread `Chat actions` menu from `local-conversation-thread-CRSaT3IN.js`. |
| Open Shell package | File browser right panel | `packages/ui/src/file-browser/FileBrowserPanel.tsx` | high | Right-panel file browser shell using the shared `FileTree`, based on `RightPanelOutlet`/`RightPanelTabs` in `thread-app-shell-chrome-BjerXYKb.js` and file search from `file-tree-search-input-DWq_lg9v.js`. |
| Open Shell package | File tree | `packages/ui/src/file-tree/FileTree.tsx` | high | Current package version keeps Codex tree data attributes, selected rows, git/action lanes, `Filter files…` search wrapper, and core icon sprite. |
| Open Shell package | Chat thread surface | `packages/ui/src/thread/ThreadSurface.tsx` | high | Conversation surface moved closer to local-thread layout: assistant activity rows, right-aligned user bubble, and edited-files review card. |
...
| 2026-06-05 | Open Shell package port continued: `FileTree` now uses the upstream `Filter files…` search wrapper and flex section lanes, `FileBrowserPanel` provides the right-side file tree/editor slot, `AppShell` has the thread actions topbar menu, and `ThreadSurface` has a denser Codex-like conversation/review-card rhythm. |
