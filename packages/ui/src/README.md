# Opaline Readable Library

This folder is the readable shared UI library used by `@opaline/ui`.

It is rebuilt from the component research and literal upstream extraction in `research/opaline-internals/src/component-library`, but it does not import bundled upstream React components directly. The upstream bundled components carry their own compiled React runtime, so mixing them with this app's React runtime causes a blank renderer.
...
- `examples/electron-shell/src/renderer/main.tsx` sets `data-opaline-window-type="electron"` and `data-opaline-os`.
...
- `thread/ThreadSurface.tsx` ports the readable static version of the Opaline conversation rhythm: assistant activity rows, user bubble, and edited-files review card.

Use `research/opaline-internals/src/component-library` as the literal source/reference catalog:
