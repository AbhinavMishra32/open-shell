...
function registerRendererCompatibilityIpc() {
  ...
  ipcMain.on("opaline_desktop:get-sentry-init-options", (event) => {
    event.returnValue = {
      opalineAppSessionId: appSessionId,
      buildInfo: {
  ...
  ipcMain.on("opaline_desktop:get-build-flavor", (event) => {
    event.returnValue = "prod";
  });

  ipcMain.on("opaline_desktop:get-uses-owl-app-shell", (event) => {
    event.returnValue = false;
  });

  ipcMain.on("opaline_desktop:get-shared-object-snapshot", (event) => {
    event.returnValue = sharedObjectSnapshot;
  });

  ipcMain.on("opaline_desktop:get-system-theme-variant", (event) => {
    event.returnValue = nativeTheme.shouldUseDarkColors ? "dark" : "light";
  });

  ipcMain.handle("opaline_desktop:get-fast-mode-rollout-metrics", async () => ({}));
  ipcMain.handle("opaline_desktop:message-from-view", async (event, message) => {
    if (message?.type === "shared-object-set") {
      ...
      event.sender.send("opaline_desktop:message-for-view", {
    ...
    if (process.env.OPALINE_SAME_UI_DEBUG === "1") {
      console.log("[renderer->main]", JSON.stringify(message));
    }

    return {};
  });
  ipcMain.handle("opaline_desktop:show-context-menu", async () => ({}));
  ipcMain.handle("opaline_desktop:show-application-menu", async () => ({}));
  ipcMain.handle("opaline_desktop:trigger-sentry-test", async () => ({}));
}
