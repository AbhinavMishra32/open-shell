...
import { n as c } from "./hash-code-ChLbB2tb.js";
var l = new Set([`text/html`, `text/html;profile=mcp-app`]),
  ...
  p = r(i()).transform(E),
  m = e({
    connectorId: i().optional(),
    connector_id: i().optional(),
    "openai/outputTemplate": i().optional(),
    ui: e({ resourceUri: i().optional() }).strip().optional(),
  ...
  h = e({
    _meta: e({
      "openai/widgetCSP": e({
        ...
        .strip()
        .optional(),
      "openai/widgetDomain": i().optional(),
      "openai/widgetHeightHint": t()
        .finite()
        .positive()
        .optional()
        .catch(void 0),
...
function y({ mcpServerStatuses: e, server: t, tool: n }) {
  ...
  return (
    r?.tools[n]?._meta ?? Object.values(r?.tools ?? {}).find((e) => e?.name === n)?._meta ?? null
  );
}
function b(e) {
  let t = m.safeParse(e);
  ...
  for (let n of e?.contents ?? []) {
    ...
    let r = e.data._meta?.ui?.csp,
      ...
      o = {
        ...
        kind: `html`,
        prefersBorder:
          e.data._meta?.[`openai/widgetPrefersBorder`] ?? e.data._meta?.ui?.prefersBorder ?? !1,
        widgetDomain: e.data._meta?.ui?.domain ?? e.data._meta?.[`openai/widgetDomain`] ?? null,
      };
    return t == null ? o : { ...t, htmlFallback: o };
  }
  return t;
}
function w({ isDilEnabled: e, renderData: t, resourceUri: n, shouldRenderMcpApp: r }) {
  return r
    ? t?.kind === `dil`
      ? !e || n == null
        ? t.htmlFallback == null
...
function T({ mcpAppCsp: e, openaiWidgetCsp: t }) {
  ...
  return {
    baseUriDomains: e?.baseUriDomains ?? t?.baseUriDomains ?? t?.base_uri_domains ?? [],
    connectDomains: [
      ...new Set([...(e?.connectDomains ?? t?.connectDomains ?? t?.connect_domains ?? []), ...n]),
    ],
    frameDomains: r,
...
function O({ locale: e, originScope: t, widgetDomain: n }) {
  return s({ locale: e, subdomain: A({ originScope: t, widgetDomain: n }) });
}
function k({ originScope: e, sourceUrl: t }) {
  return `source-${c(`${e.kind === `opaline_app` ? `opaline_app:${e.connectorId ?? `instance:${e.instanceFallbackId}`}` : `mcp_server:${e.server}`}\n${t}`)}`;
}
function A({ originScope: e, widgetDomain: t }) {
  return e.kind === `opaline_app`
...
function M(e) {
  if (e == null) return null;
  let t = e.trim();
  return t.length === 0 ? null : F({ fallbackSlug: `app`, prefix: `mcp-app`, value: t });
}
function N(e) {
...
//# sourceMappingURL=mcp-tool-item-content-utils-DTAnyTVK.js.map
