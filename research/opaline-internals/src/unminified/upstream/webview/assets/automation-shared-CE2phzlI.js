import { Ai as e, Di as t, Gn as n, ji as r } from "./src-CYrn1fYL.js";
import { at as i } from "./vscode-api-sUstfl-x.js";
import { i as a, m as o, p as s, r as c, t as l, u } from "./automation-schedule-Cko2eDIZ.js";
var d = {
    id: null,
    kind: `cron`,
    name: ``,
...
function p(e) {
  ...
  return (
    ...
    u(e.scheduleConfig) || r.push(`schedule`),
    { trimmedName: t, trimmedPrompt: n, missingRequirements: r, canSave: r.length === 0 }
  );
}
function m(e) {
  return !e.scheduleDirty && e.rawRrule ? e.rawRrule : l(e.scheduleConfig);
}
function h(e, t) {
  return {
    ...e,
...
var g = i(null),
  _ = i(null),
  v = i(!1);
function y({ directiveKey: e, automation: t }) {
  return {
    directiveKey: e,
    mode: `view`,
    id: t.id,
...
function b(n, i) {
  ...
  return {
    ...
    status: n.status,
    cwds: e(n) ? [] : n.cwds,
    executionEnvironment: e(n) ? null : r(n.executionEnvironment),
    localEnvironmentConfigPath: e(n) ? null : n.localEnvironmentConfigPath,
    targetThreadId: e(n) ? n.targetThreadId : null,
    model: e(n) ? null : a.model,
    reasoningEffort: e(n) ? null : a.reasoningEffort,
    rawRrule: n.rrule,
    scheduleConfig: e(n) ? s(n.rrule) : o(n.rrule),
    scheduleDirty: !1,
  };
}
function x({ seed: i, targetAutomation: a, models: c }) {
  let l = i.mode === `view` ? a : null,
    u = l?.rrule ?? i.rrule,
    f = a == null ? null : t({ automation: a, models: c ?? [] }),
    p =
      i.id != null && a == null
        ? null
        : a != null && !e(a)
          ? r(a.executionEnvironment)
          : d.executionEnvironment,
    m = a?.kind ?? i.kind ?? (i.id != null && a == null ? d.kind : `cron`),
    h =
      m === `heartbeat`
        ? (i.targetThreadId ?? (a != null && e(a) ? a.targetThreadId : null))
        : null;
  return {
    id: i.id ?? a?.id ?? null,
    kind: m,
    name: l?.name ?? i.name,
    prompt: l?.prompt ?? i.prompt,
    status: l?.status ?? i.status ?? a?.status ?? `ACTIVE`,
    cwds: a != null && !e(a) ? a.cwds : i.cwds.map(n),
    executionEnvironment: m === `heartbeat` ? null : (i.executionEnvironment ?? p),
    localEnvironmentConfigPath:
      m === `heartbeat`
        ? null
        : (i.localEnvironmentConfigPath ??
          (a != null && !e(a) ? a.localEnvironmentConfigPath : d.localEnvironmentConfigPath)),
    targetThreadId: h,
    model: m === `heartbeat` ? null : (i.model ?? f?.model ?? d.model),
    reasoningEffort:
      m === `heartbeat` ? null : (i.reasoningEffort ?? f?.reasoningEffort ?? d.reasoningEffort),
    rawRrule: u,
    scheduleConfig: m === `heartbeat` ? s(u) : o(u),
    scheduleDirty: !1,
  };
}
function S(e) {
...
function T({ allowThreadDestination: e }) {
...
function D(e, t) {
  let n = t;
  t.includes(f) && (n = P(e.cwds) && t.length > 1 ? t.filter((e) => e !== f) : [f]);
...
function O({ draft: e, threadId: t, title: n }) {
  return { ...e, name: e.name.trim().length === 0 ? n : e.name, targetThreadId: t };
}
function k(e) {
  return e.kind === `cron` && P(e.cwds);
}
function A({ draft: e, modelSettings: t }) {
...
function j({ draft: e, name: t, prompt: n, status: r, rrule: i }) {
...
function M({ draft: e, name: t, prompt: n, rrule: r }) {
  if (e.kind === `heartbeat`) {
    if (e.targetThreadId == null) throw Error(`Heartbeat automation draft is incomplete`);
    return {
      kind: `heartbeat`,
...
function N({ directiveMode: e, canCreate: t, canUpdate: n, isViewMode: r, forceOpen: i = !1 }) {
  return i
    ? `open`
    : e === `view`
      ? r
...
//# sourceMappingURL=automation-shared-CE2phzlI.js.map
