import { s as e } from "./chunk-Bj-mKKzh.js";
import { n as t, t as n } from "./jsx-runtime-CiQ1k8xo.js";
import { t as r } from "./clsx-BcPLHiun.js";
import { lt as i } from "./vscode-api-sUstfl-x.js";
import { n as a } from "./react-v2Nz4MGb.js";
import { t as o } from "./persisted-atom-C-xVMviv.js";
import { t as s } from "./proxy-DktXrBt_.js";
import { t as c } from "./use-reduced-motion-BsrTWbI1.js";
import { t as l } from "./opaline-avatar-9OkGRaoz.js";
var u = i(),
  d = e(t(), 1),
  f = n(),
  p = 4,
  m = {
    "top-start": `top-0 left-0`,
    "top-end": `top-0 right-0`,
    "bottom-start": `bottom-0 left-0`,
    "bottom-end": `right-7 bottom-0`,
  },
  h = o(`avatar-mascot-notification-badge-corner`, `top-end`);
function g(e) {
  let t = (0, u.c)(23),
    {
      ariaLabel: n,
      assetRef: i,
      className: a,
      notificationBadge: o,
      onContextMenu: s,
      resizeHandle: c,
      spritesheetUrl: p,
      state: m,
      style: h,
      transientState: g,
    } = e,
    v = m === void 0 ? `idle` : m,
    [y, b] = (0, d.useState)(!1),
    x = g ?? (y ? `jumping` : v),
    S = o != null,
    C = S || c != null,
    w;
  n != null && (w = C ? `group` : `img`);
  let T;
  t[0] === a
    ? (T = t[1])
    : ((T = r(
        `opaline-avatar-button relative flex cursor-interaction items-center justify-center active:cursor-grabbing`,
        a,
      )),
      (t[0] = a),
      (t[1] = T));
  let E = n == null && !C ? !0 : void 0,
    D,
    O;
  t[2] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((D = () => {
        b(!0);
      }),
      (O = () => {
        b(!1);
      }),
      (t[2] = D),
      (t[3] = O))
    : ((D = t[2]), (O = t[3]));
  let k;
  t[4] !== i || t[5] !== x || t[6] !== p
    ? ((k = (0, f.jsx)(l, {
        assetRef: i,
        className: `relative z-10`,
        spritesheetUrl: p,
        state: x,
      })),
      (t[4] = i),
      (t[5] = x),
      (t[6] = p),
      (t[7] = k))
    : (k = t[7]);
  let A;
  t[8] !== S || t[9] !== o
    ? ((A = S ? (0, f.jsx)(_, { notificationBadge: o }) : null),
      (t[8] = S),
      (t[9] = o),
      (t[10] = A))
    : (A = t[10]);
  let j;
  t[11] === c
    ? (j = t[12])
    : ((j =
        c == null
          ? null
          : (0, f.jsx)(`div`, {
              className: `group absolute right-0 bottom-0 z-30 flex size-12 cursor-default items-end justify-end rounded-[8px] text-token-text-secondary hover:text-token-foreground`,
              "data-testid": `avatar-overlay-resize-hover-target`,
              children: (0, f.jsx)(`button`, {
                type: `button`,
                "aria-label": c.ariaLabel,
                className: `no-drag opaline-avatar-resize-handle flex size-5 cursor-nwse-resize touch-none items-center justify-center rounded-[6px] border border-token-border-default/80 bg-token-bg-primary p-1 opacity-0 shadow-lg shadow-black/20 backdrop-blur-sm group-hover:opacity-100 focus-visible:opacity-100 focus-visible:ring-2 focus-visible:ring-token-focus-border focus-visible:outline-none forced-colors:bg-[Canvas] forced-colors:backdrop-blur-none [@media(prefers-reduced-transparency:reduce)]:backdrop-blur-none`,
                "data-testid": `avatar-overlay-resize-handle`,
                onLostPointerCapture: c.onLostPointerCapture,
                onPointerCancel: c.onPointerCancel,
                onPointerDown: c.onPointerDown,
                onPointerEnter: c.onPointerEnter,
                onPointerLeave: c.onPointerLeave,
                onPointerMove: c.onPointerMove,
                onPointerUp: c.onPointerUp,
                children: (0, f.jsx)(`svg`, {
                  viewBox: `0 0 12 12`,
                  className: `size-3`,
                  "data-testid": `avatar-overlay-resize-icon`,
                  children: (0, f.jsx)(`path`, {
                    d: `M1.75 5V1.75H5M7 10.25h3.25V7M2 2l8 8`,
                    fill: `none`,
                    stroke: `currentColor`,
                    strokeLinecap: `round`,
                    strokeLinejoin: `round`,
                    strokeWidth: `1.5`,
                  }),
                }),
              }),
            })),
      (t[11] = c),
      (t[12] = j));
  let M;
  return (
    t[13] !== n ||
    t[14] !== s ||
    t[15] !== w ||
    t[16] !== h ||
    t[17] !== T ||
    t[18] !== E ||
    t[19] !== k ||
    t[20] !== A ||
    t[21] !== j
      ? ((M = (0, f.jsxs)(`div`, {
          className: T,
          "data-avatar-mascot": `true`,
          "data-testid": `avatar-mascot-button`,
          "aria-hidden": E,
          "aria-label": n,
          role: w,
          onContextMenu: s,
          onPointerEnter: D,
          onPointerLeave: O,
          style: h,
          children: [k, A, j],
        })),
        (t[13] = n),
        (t[14] = s),
        (t[15] = w),
        (t[16] = h),
        (t[17] = T),
        (t[18] = E),
        (t[19] = k),
        (t[20] = A),
        (t[21] = j),
        (t[22] = M))
      : (M = t[22]),
    M
  );
}
function _(e) {
  let t = (0, u.c)(35),
    { notificationBadge: n } = e,
    [i, o] = a(h),
    [l, g] = (0, d.useState)(null),
    _ = (0, d.useRef)(null),
    y = (0, d.useRef)(!1),
    b = c(),
    x;
  t[0] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((x = (e) => {
        e.button === 0 &&
          (e.stopPropagation(),
          e.currentTarget.setPointerCapture?.(e.pointerId),
          (_.current = {
            hasMoved: !1,
            pointerId: e.pointerId,
            startClientX: e.clientX,
            startClientY: e.clientY,
          }));
      }),
      (t[0] = x))
    : (x = t[0]);
  let S = x,
    C;
  t[1] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((C = (e) => {
        let t = _.current;
        if (t == null || t.pointerId !== e.pointerId) return;
        e.stopPropagation();
        let n = e.clientX - t.startClientX,
          r = e.clientY - t.startClientY;
        (!t.hasMoved && Math.abs(n) < p && Math.abs(r) < p) ||
          (e.preventDefault(), (t.hasMoved = !0), g({ x: n, y: r }));
      }),
      (t[1] = C))
    : (C = t[1]);
  let w = C,
    T;
  t[2] === o
    ? (T = t[3])
    : ((T = (e) => {
        let t = _.current;
        if (
          !(t == null || t.pointerId !== e.pointerId) &&
          (e.stopPropagation(),
          (_.current = null),
          e.currentTarget.hasPointerCapture?.(e.pointerId) &&
            e.currentTarget.releasePointerCapture?.(e.pointerId),
          t.hasMoved)
        ) {
          let t = e.currentTarget.parentElement?.getBoundingClientRect();
          (t != null && o(v({ avatarBounds: t, clientX: e.clientX, clientY: e.clientY })),
            g(null),
            e.preventDefault(),
            (y.current = !0),
            window.setTimeout(() => {
              y.current = !1;
            }, 0));
        }
      }),
      (t[2] = o),
      (t[3] = T));
  let E = T,
    D;
  t[4] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((D = (e) => {
        let t = _.current;
        t == null ||
          t.pointerId !== e.pointerId ||
          (e.stopPropagation(),
          (_.current = null),
          g(null),
          e.currentTarget.hasPointerCapture?.(e.pointerId) &&
            e.currentTarget.releasePointerCapture?.(e.pointerId));
      }),
      (t[4] = D))
    : (D = t[4]);
  let O = D,
    k;
  t[5] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((k = (e) => {
        let t = _.current;
        t == null || t.pointerId !== e.pointerId || ((_.current = null), g(null));
      }),
      (t[5] = k))
    : (k = t[5]);
  let A = k,
    j;
  t[6] === n
    ? (j = t[7])
    : ((j = (e) => {
        if ((e.stopPropagation(), y.current)) {
          ((y.current = !1), e.preventDefault());
          return;
        }
        n.onClick();
      }),
      (t[6] = n),
      (t[7] = j));
  let M = j,
    N;
  t[8] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((N = { opacity: 1, scale: 1, y: 0 }), (t[8] = N))
    : (N = t[8]);
  let P = n.ariaLabel,
    F = m[i],
    I = n.isIconOnly ? `size-7 p-0` : `min-h-7 min-w-7 px-2 py-1`,
    L;
  t[9] !== I || t[10] !== F
    ? ((L = r(
        `no-drag absolute z-40 flex cursor-grab touch-none select-none items-center justify-center rounded-full border border-token-border/60 text-xs leading-none font-medium shadow-sm active:cursor-grabbing focus-visible:ring-2 focus-visible:ring-token-focus-border focus-visible:outline-none`,
        F,
        I,
      )),
      (t[9] = I),
      (t[10] = F),
      (t[11] = L))
    : (L = t[11]);
  let R = b ? 1 : 0.7,
    z = b ? 0 : 3,
    B;
  t[12] !== R || t[13] !== z
    ? ((B = { opacity: 0, scale: R, y: z }), (t[12] = R), (t[13] = z), (t[14] = B))
    : (B = t[14]);
  let V = l == null ? void 0 : `${l.x}px ${l.y}px`,
    H;
  t[15] !== n.backgroundColor || t[16] !== n.foregroundColor || t[17] !== V
    ? ((H = { backgroundColor: n.backgroundColor, color: n.foregroundColor, translate: V }),
      (t[15] = n.backgroundColor),
      (t[16] = n.foregroundColor),
      (t[17] = V),
      (t[18] = H))
    : (H = t[18]);
  let U, W;
  t[19] === b
    ? ((U = t[20]), (W = t[21]))
    : ((U = b ? { duration: 0 } : { damping: 20, mass: 0.7, stiffness: 420, type: `spring` }),
      (W = b ? void 0 : { scale: 1.06 }),
      (t[19] = b),
      (t[20] = U),
      (t[21] = W));
  let G;
  t[22] === b ? (G = t[23]) : ((G = b ? void 0 : { scale: 0.94 }), (t[22] = b), (t[23] = G));
  let K;
  return (
    t[24] !== M ||
    t[25] !== E ||
    t[26] !== n.ariaLabel ||
    t[27] !== n.content ||
    t[28] !== L ||
    t[29] !== B ||
    t[30] !== H ||
    t[31] !== U ||
    t[32] !== W ||
    t[33] !== G
      ? ((K = (0, f.jsx)(s.button, {
          type: `button`,
          animate: N,
          "aria-label": P,
          className: L,
          "data-testid": `avatar-overlay-notification-badge`,
          initial: B,
          onClick: M,
          onLostPointerCapture: A,
          onPointerCancel: O,
          onPointerDown: S,
          onPointerMove: w,
          onPointerUp: E,
          style: H,
          transition: U,
          whileHover: W,
          whileTap: G,
          children: n.content,
        })),
        (t[24] = M),
        (t[25] = E),
        (t[26] = n.ariaLabel),
        (t[27] = n.content),
        (t[28] = L),
        (t[29] = B),
        (t[30] = H),
        (t[31] = U),
        (t[32] = W),
        (t[33] = G),
        (t[34] = K))
      : (K = t[34]),
    K
  );
}
function v({ avatarBounds: e, clientX: t, clientY: n }) {
  return `${n < e.top + e.height / 2 ? `top` : `bottom`}-${t < e.left + e.width / 2 ? `start` : `end`}`;
}
export { g as t };
//# sourceMappingURL=avatar-mascot-button-CnbIqNGL.js.map
