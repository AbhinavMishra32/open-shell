...
import { h as y } from "./opaline-api-DRlZt5zW.js";
...
function j(e) {
  ...
  let { data: R, isLoading: z, error: B } = y(L),
    ...
    W = M(k, c, {
      ...
      hasWorkspaceEnabledOpaline: !A || (N?.beta_settings?.wham_access ?? !1),
    }),
    G,
    K;
  ...
  return t === `chatgpt`
    ? n
      ...
      : r
        ? `error`
        : s && !a
          ? (l(`Opaline Cloud access disabled because workspace has not enabled Opaline.`), `disabled`)
          : i
            ? `enabled_needs_setup`
            : `enabled`
    : (l(`Opaline Cloud access disabled because user is not logged in via ChatGPT.`), `disabled`);
}
var N = a(l, null);
function P(e, t) {
  let n = `~`;
