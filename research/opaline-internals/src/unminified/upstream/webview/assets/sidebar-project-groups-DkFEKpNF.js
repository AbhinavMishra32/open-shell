...
import { Ar as n, U as r, rn as i } from "./src-CYrn1fYL.js";
import { Ho as a, No as o, gc as s, wn as c } from "./app-server-manager-signals-BOGyjFm3.js";
import { g as l } from "./vscode-api-sUstfl-x.js";
import { t as u } from "./_baseEach-BGrGeeUo.js";
import { n as d } from "./parse-owner-repo-BIB9Pu_g.js";
import { n as f } from "./remote-projects-nghNpfPT.js";
import { t as p } from "./_defineProperty-b-BZyUPb.js";
var m = t((e, t) => {
    function n(e) {
...
function O(e, t, i) {
  let a = (0, b.default)(t.map((e) => [n(e.dir), e]));
  return (e?.roots ?? []).map((o) => {
    let s = n(o),
      c = r(o, i),
      l = (e?.labels?.[o] ?? e?.labels?.[s] ?? ``).trim() || (0, S.default)(I(o)) || ``,
      u = a[s] ?? void 0,
      f = u?.originUrl ? d(u.originUrl) : null;
    return u && A(u)
      ? {
          projectId: o,
          projectKind: `local`,
...
function M(e, t, r, i, a, o) {
  let s = (0, b.default)(
    ...
    l = new Map();
  for (let e of r) e.projectKind === `local` && L(e) && l.set(n(e.path), e);
  return (
    e.forEach((e) => {
      if (e.kind === `local`) B(e, r, l, i, o?.gitOriginsByHostId, a, o?.primaryHostId, o);
      else if (e.kind === `remote`) {
        let t = V(o?.threadProjectAssignments?.[e.task.id], r);
        if (t != null) {
        ...
        Y(e, c, r);
      } else e.kind === `pending-worktree` && J(e, s, r, l);
    }),
    r
  );
}
...
function P(e, t) {
  let r = n(e).replace(/\/+$/, ``);
  return t.find((e) => n(e.dir).replace(/\/+$/, ``) === r) ?? null;
}
function F(e, t) {
  let r = P(e, t ?? []);
  if (!r?.root) return [];
...
function R(e, t) {
  return e.get(n(t)) ?? null;
}
function z(e, t) {
  return e.has(n(t));
}
var B = (e, t, n, i, o, s, c = a, u) => {
  let d = e.cwd;
  if (!d || !I(d).length) {
  ...
  let p = d,
    m = e.hostId,
    h = u?.threadProjectAssignments?.[e.conversationId],
    g =
      h != null &&
      (h.projectKind === `local`
  ...
  if (e.workspaceKind === `projectless` || u?.projectlessThreadIds?.has(e.conversationId) === !0)
    return;
  let _ = m != null && m !== `local`,
    v = u?.remoteProjects;
  if (
    _ &&
    ((u?.enabledRemoteHostIds && !u.enabledRemoteHostIds.has(m)) || !v?.some((e) => e.hostId === m))
  )
    return;
  let y = W({ gitOrigins: i, gitOriginsByHostId: o, hostId: m ?? void 0, primaryHostId: c });
  if (r(d, s) || (_ && G(d, y))) {
    let r = K(d, e.conversationId, t, n, y, u?.threadWorkspaceRootHints);
    r && (p = r);
  }
  let b = v == null ? null : f(v, m ?? void 0, p);
  if (b != null) {
  ...
  x &&
    ...
    p !== d && u?.onDiscoverThreadWorkspaceRootHint?.(e.conversationId, x.path));
};
function V(e, t) {
  return e == null
    ? null
...
function U(e, t) {
  let r = new Map(t.map(({ hostId: e, dirs: t }) => [e, new Set(t.map(n))]));
  return e
    .map(({ hostId: e, dirs: t }) => ({ hostId: e, dirs: t.filter((t) => !r.get(e)?.has(n(t))) }))
...
function G(e, t) {
  let r = P(e, t ?? []);
  return r?.commonDir
    ? n(r.commonDir).replace(/\/+$/, ``) !== `${n(r.root).replace(/\/+$/, ``)}/.git`
    : !1;
}
function K(e, t, r, i, a, o) {
  if (z(i, e)) return null;
  let s = o?.[t],
    c = s ? R(i, s) : null;
  ...
  let u = l.originUrl ?? null,
    d = l.commonDir ?? null;
  if (!u && !d) return null;
  let f = (e) => (e ? (u ? e.originUrl === u : d ? e.commonDir === d : !1) : !1),
    p = n(e),
    m = F(e, a).join(`/`),
    h = r.flatMap((e) => {
      if (!L(e)) return [];
      let t = n(e.path);
      if (e.isOpalineWorktree && p !== t) return [];
      let r = P(e.path, a);
      return r != null && f(r) ? [e] : [];
    }),
    g = h.filter((e) => e.repositoryData?.repoPath === m),
    _ = (0, S.default)(I(l.root)) ?? ``,
    v = q(g, _);
  if (v) return v.path;
  let y = g[0];
  if (y) return y.path;
  ...
  let s = t[n(o)] ?? null,
    c =
      a.hostId === `local`
        ? R(i, o)
        : (r.find(
            (e) => e.projectKind === `remote` && e.hostId === a.hostId && n(e.path) === n(o),
          ) ?? null);
  c &&
    ((s && c.repositoryData?.ownerRepo != null && !N(c.repositoryData.ownerRepo, s)) ||
...
//# sourceMappingURL=sidebar-project-groups-DkFEKpNF.js.map
