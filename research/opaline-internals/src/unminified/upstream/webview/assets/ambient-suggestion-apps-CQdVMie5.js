...
import { P as e, R as t, h as n, lt as r } from "./vscode-api-sUstfl-x.js";
import { n as i } from "./lib-MoKmYgcO.js";
import { t as a } from "./known-app-icon-BEI7OXDu.js";
import { t as o } from "./use-auth-DXLjX3T4.js";
import { f as s, t as c } from "./apps-queries-CBUjthvi.js";
import { r as l } from "./skus-CJAzlvMK.js";
var u = 3,
  d = i({
    id: `home.connectAppsRow`,
    defaultMessage: `Connect your favorite apps to Opaline`,
...
function p(e) {
  return e === `default` ? `engineering` : e in f ? e : `something_else`;
}
function m(e) {
  let t = e.length > 0 ? e : [`something_else`];
  return Array.from(new Set(t.map(p)));
}
function h({ roles: e }) {
  return {
    connectAppsRowMessage: d,
    role: p(e[0] ?? `something_else`),
    suggestionPrompts: g({ roles: e, promptsByRole: f }),
  };
}
function g({ roles: e, promptsByRole: t, limit: n = u }) {
  let r = m(e).map((e) => t[e]),
    i = r.map(() => 0),
    a = [],
    o = new Set();
  for (let [e, t] of r.entries()) {
    let r = t[i[e]];
    if (((i[e] += 1), !(!r || o.has(r)) && (o.add(r), a.push(r), a.length >= n))) return a;
  }
  for (; a.length < n; ) {
    let e = !1;
    for (let t = 0; t < r.length; t += 1) {
      let s = r[t][i[t]];
      if (((i[t] += 1), !(!s || o.has(s)) && (o.add(s), a.push(s), (e = !0), a.length >= n))) break;
    }
    if (!e) break;
  }
  return a;
...
function M({ intl: e, mailProvider: t, plan: n = null, plugins: r = [], roles: i }) {
  let a = i[0] ?? `something_else`,
    o = t === `other` ? (l(n) ? `microsoft` : `google`) : t;
  return j[a].flatMap((t) => {
...
function L(t) {
  let n = (0, N.c)(8),
    { appIds: r, enabled: i, hostId: a } = t,
    c = i === void 0 ? !0 : i,
    l = a ?? `local`,
    u = o(),
    d;
  n[0] === l ? (d = n[1]) : ((d = { hostId: l }), (n[0] = l), (n[1] = d));
  let f = s(d),
    p = c && f && !u.isLoading && u.userId != null,
    m;
  n[2] !== l || n[3] !== p
    ? ((m = { hostId: l, isQueryEnabled: p }), (n[2] = l), (n[3] = p), (n[4] = m))
    : (m = n[4]);
  let h = e(F, m);
  if (h == null) return;
  let g;
  return (
    n[5] !== h || n[6] !== r ? ((g = I(h, r)), (n[5] = h), (n[6] = r), (n[7] = g)) : (g = n[7]),
    g
  );
}
function R(e) {
...
export { L as a, h as c, R as i, P as n, M as o, I as r, d as s, z as t };
//# sourceMappingURL=ambient-suggestion-apps-CQdVMie5.js.map
