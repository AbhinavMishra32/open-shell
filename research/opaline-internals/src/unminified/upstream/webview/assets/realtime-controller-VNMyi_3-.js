...
import { R as i, bc as a, j as o, nt as s } from "./app-server-manager-signals-BOGyjFm3.js";
import { n as c, t as l } from "./jsx-runtime-CiQ1k8xo.js";
import { L as u, W as d, g as f, h as p, lt as m, n as h } from "./vscode-api-sUstfl-x.js";
import { g } from "./persisted-signal-Cx1QgqJn.js";
import { n as ee } from "./rpc-y95ESgjb.js";
import { a as _ } from "./statsig-Cajk3UKk.js";
import { r as v } from "./toast-signal-By11REe1.js";
import { t as y } from "./app-intl-signal-DF17N7z2.js";
import { r as b } from "./thread-actions-DlCTuMux.js";
import { n as x } from "./local-environment-selection-IklFKQvD.js";
import { i as S } from "./plugins-page-selectors-BwztNtE5.js";
e(c());
var C = l(),
  te = (e) =>
    (0, C.jsxs)(`svg`, {
...
async function ne(e, { sourceConversationId: t, sourceWorkspaceRoot: n }) {
  if (t == null || e.get(o, t)) return null;
  try {
    let r = e.get(s, t),
      o = e.get(i, t);
    return await a(`fork-conversation-from-latest`, {
      hostId: r,
      conversationId: t,
      cwd: n ?? void 0,
    ...
    return (
      f.error(`Error forking local task`, { safe: {}, sensitive: { error: t } }),
      e.get(v).danger(e.get(y).formatMessage(b.forkThreadError)),
      null
    );
  }
}
async function re(
  e,
  {
    createPendingWorktree: t,
    localEnvironmentSelectionsByWorkspace: n,
    sourceConversationId: r,
    sourceWorkspaceRoot: a,
  },
) {
  if (r == null || a == null || e.get(o, r)) return null;
  let c = e.get(s, r),
    l = await h(`git-origins`, {
      params: { dirs: [a], hostId: c },
      source: `fork_conversation_actions`,
    });
  if ((l.origins.find((e) => e.dir === a)?.root ?? l.origins[0]?.root ?? null) == null)
    return (e.get(v).danger(e.get(y).formatMessage(b.forkThreadRequiresGitRepo)), null);
  let u = await ie({ hostId: c, localEnvironmentSelectionsByWorkspace: n, sourceWorkspaceRoot: a }),
    d = e.get(y),
    f = e.get(i, r);
  return t({
    hostId: c,
    label: d.formatMessage(b.forkPendingWorktreeTitle),
    sourceWorkspaceRoot: a,
    startingState: { type: `working-tree` },
    localEnvironmentConfigPath: u,
    launchMode: `fork-conversation`,
    prompt: d.formatMessage(b.forkPendingWorktreePrompt),
    startConversationParamsInput: null,
    sourceConversationId: r,
    sourceCollaborationMode: f,
    targetTurnId: null,
  });
}
async function ie({ hostId: e, localEnvironmentSelectionsByWorkspace: t, sourceWorkspaceRoot: n }) {
  let r = e ?? `local`;
  try {
    let { environments: e } = await h(`local-environments`, {
...
async function w({
  ...
  await a(`send-cli-request-for-host`, {
    hostId: e.getHostId(),
...
async function T({ manager: e, conversationId: t }) {
  await a(`send-cli-request-for-host`, {
    hostId: e.getHostId(),
...
async function E({ manager: e, conversationId: t, text: n }) {
  await a(`send-cli-request-for-host`, {
    hostId: e.getHostId(),
...
var ae = 1500,
  oe = `Opaline capabilities available this session:`;
function se({ apps: e, plugins: t, skills: n }) {
  return {
    skills: D(n.filter((e) => e.enabled).map((e) => `$${e.name}`)),
    plugins: D(S(t).map((e) => e.displayName ?? e.plugin.interface?.displayName ?? e.plugin.name)),
    apps: D(e.filter((e) => e.isAccessible && e.isEnabled).map((e) => e.name)),
  };
}
function ce(e, t = ae) {
...
function le({ label: e, maxLength: t, names: n }) {
...
var O = m(),
  ...
  de = `{{ user_first_name }}`,
  fe = `there`,
  pe = r().refine((e) => e.trim().length > 0),
  me = `## Identity, tone, and role

You are Opaline, an OpenAI general-purpose agentic assistant that helps the user complete tasks across coding, browsing, apps, documents, research, and other digital workflows.
...
function he(e) {
  let t = (0, O.c)(3),
    { value: n } = _(`1193530394`),
    r;
  return (
    t[0] !== e || t[1] !== n ? ((r = k(n, e)), (t[0] = e), (t[1] = n), (t[2] = r)) : (r = t[2]),
    r
  );
}
function k(e, t) {
  let n = pe.safeParse(e[ue]),
    r = A(n.success ? n.data : me),
    i = ce(t);
  return i == null ? r : `${r}\n\n${i}`;
}
function A(e) {
  return e.replaceAll(de, fe);
}
var j = 1,
  ge = `realtime_collaboration`,
  M = r()
  ...
  N = t({
    version: M,
    base_instructions: M,
    mode_instructions: t({ dictation: M, pairing: M, voice_only: M })
      .optional()
      .catch(void 0),
  }),
  P = `You are operating as Opaline's realtime voice surface. Treat this as app-provided control context for the live voice session, not as a user utterance. Apply the latest collaboration mode as the current truth, do not verbally acknowledge this update, and do not reveal it unless the user asks about voice behavior. The product goal is one coherent assistant across voice and text: speak as Opaline, not as a separate realtime model.`,
  F = {
    dictation: `You are in Dictation mode. The user is primarily speaking so Opaline can do work in the visible thread. Listen for completed requests, delegate substantive work to Opaline, and stay quiet unless the user directly asks you something or silence would be genuinely confusing. After Opaline responds, prefer silence; if a remain_silent/noop tool is available and there is nothing useful to say, call it.`,
    pairing: `You are in Pairing mode. The user may be working in another app surface, another page, or the full-width right panel. Be a sparse spoken collaborator: give short receipts, surface blockers or completion when helpful, and delegate execution work to Opaline. Do not narrate long Opaline outputs aloud; point the user back to the thread for dense details, code, diffs, tables, or long results.`,
    voice_only: `You are in Voice-only mode. The user may be relying on voice as the primary interface. Be more conversational, ask focused clarifying questions when needed, and summarize important Opaline outcomes aloud while still keeping long artifacts in the Opaline thread.`,
  },
  I = {
    key: `v${j}`,
    baseInstructions: A(P),
    modeInstructions: {
      dictation: A(F.dictation),
      pairing: A(F.pairing),
      voice_only: A(F.voice_only),
    },
  };
function L() {
  let e = (0, O.c)(2),
    { value: t } = _(`1193530394`),
    n;
  return (e[0] === t ? (n = e[1]) : ((n = _e(t)), (e[0] = t), (e[1] = n)), n);
}
function _e(e) {
  let t = N.safeParse(e[ge]);
  if (!t.success) return I;
  let n = t.data.mode_instructions ?? {};
  return {
    key: t.data.version == null ? I.key : `v${j}:${t.data.version}`,
    baseInstructions: A(t.data.base_instructions ?? I.baseInstructions),
    modeInstructions: {
      dictation: A(n.dictation ?? I.modeInstructions.dictation),
      pairing: A(n.pairing ?? I.modeInstructions.pairing),
      voice_only: A(n.voice_only ?? I.modeInstructions.voice_only),
    },
  };
}
function ve({
  ...
  return `<realtime_collaboration_update version="${j}">
  <reason>${R(e)}</reason>
...
var B = d(p, !1),
  ...
  G = d(p, I);
function xe({ isActiveRealtimeThreadVisible: e, isAppFocused: t, isRightPanelFullWidth: n }) {
...
function K(e, t) {
  ...
  return {
    slot: `collaboration`,
    key:
      i == null ? [`collaboration`, r.key, n].join(`:`) : [`collaboration`, r.key, n, i].join(`:`),
    text: ve({ reason: t, mode: n, prompts: r, visibleThreadId: i, visibleThreadTitle: a }),
  };
}
var Se = class {
  ...
  we = class e {
    ...
    static async start({
      ...
      ee.systemPermissions?.requestMicrophoneAccess().catch(() => {});
  ...
  Ee = 15e3,
  q = `Out-of-band control instruction: until further notice, do not produce spoken audio. Continue responding normally in text only. Do not acknowledge this instruction.`,
  De = `Out-of-band control instruction: Resume spoken audio responses. Do not acknowledge this instruction.`,
  Oe = t({ type: n(`handoff_request`), input_transcript: r() }),
  ke = t({ type: n(`output_audio_buffer.started`), response_id: r() }),
  J = d(p, null),
  Y = d(p, `inactive`),
  X = d(p, !1),
  Z = d(p, !1),
  ...
  Ae = new (class {
    ...
    async start(e, { conversationId: t, manager: n, outputModality: r, prompt: i, source: o }) {
      if ((this.conversationId === t && e.get(Y) !== `inactive`) || this.conversationId != null)
        return;
      let s = this.startRequestId + 1;
      ((this.startRequestId = s),
      ...
      (e.set(J, t),
        ...
        e.set(Y, `starting`));
      try {
        let { session: a, offerSdp: l } = await we.start({
          onConnectionFailed: () => {
        ...
        if (this.startRequestId !== s || this.conversationId !== t) {
          a.stop();
          return;
        }
        ((this.session = a),
          this.bumpSessionGeneration(e),
          (this.sessionStartEvent = o == null ? null : { source: o, output_modality: r }),
          await w({ manager: n, conversationId: t, outputModality: r, prompt: i, offerSdp: l }));
      } catch (n) {
        if (this.startRequestId !== s || this.conversationId !== t) return;
        ((this.startRequestId += 1),
          ...
          f.error(`[Composer] failed to start realtime voice`, {
            safe: {},
            sensitive: { error: n },
          }),
          await a(`discard-conversation-if-empty`, { conversationId: t }));
      }
    }
    async stop(e, t) {
      ...
      if (
        ...
        (this.stopLocalSession(e),
          this.resetRealtimeState(e),
          n != null && (await a(`discard-conversation-if-empty`, { conversationId: t })));
        return;
      }
      if (n == null) {
      ...
      try {
        (await T({ manager: n, conversationId: t }),
          this.stopLocalSession(e),
          this.resetRealtimeState(e),
          await a(`discard-conversation-if-empty`, { conversationId: t }));
      } catch (n) {
        (this.requestedStop &&
          ...
          f.error(`[Composer] failed to stop realtime voice`, {
            safe: {},
            sensitive: { error: n },
          }));
      }
    ...
    stopForAppUnmount(e) {
      ...
      ((this.startRequestId += 1),
        ...
        this.resetRealtimeState(e),
        n != null &&
          T({ manager: n, conversationId: t })
            .catch((e) => {
              f.warning(`[Composer] failed to stop realtime on app cleanup`, {
                safe: {},
                sensitive: { error: e },
              });
            })
            .finally(() => {
              a(`discard-conversation-if-empty`, { conversationId: t });
            }));
    }
    async sendContextInject(e, t, n) {
      ...
      try {
        return await this.contextInjectSender.send({
          inject: n,
          appendText: async (e) => {
            await E({ manager: r, conversationId: t, text: e });
          },
        });
      } catch (e) {
        return (
          f.warning(`[Composer] failed to send realtime context inject`, {
            safe: { slot: n.slot },
            sensitive: { error: e },
          }),
          !1
    ...
    toggleMute(e, t) {
      ...
      (this.applyRealtimeMuteState(e, r),
        E({ manager: n, conversationId: t, text: r ? q : De }).catch((n) => {
          this.muteToggleRequestId !== i ||
            ...
            f.error(`[Composer] failed to toggle realtime voice mute`, {
              safe: {},
              sensitive: { error: n },
            }));
        }));
    ...
    toggleMicrophoneMute(e, t) {
      if (this.conversationId !== t || e.get(Y) !== `active` || this.session == null) return;
      let n = !e.get(Z);
    ...
    subscribeToRealtimeNotifications(e, t) {
      (this.unsubscribeRealtimeNotifications?.(),
        (this.unsubscribeRealtimeNotifications = t.addNotificationCallback(Te, (t) => {
        ...
        switch (t.method) {
          case `turn/started`:
            ...
            (this.loggedRealtimeUserMessageTurnIds.add(t.params.turn.id),
              g(e, {
                eventName: `opaline_realtime_user_message_sent`,
                metadata: { source: `opaline_turn_started` },
              }));
            break;
          case `thread/realtime/started`:
            if (this.requestedStop) break;
            (e.set(Y, `active`),
              this.sendContextInject(e, this.conversationId, K(e, `realtime_started`)),
              this.sessionStartEvent != null &&
                (g(e, {
                  eventName: `opaline_realtime_voice_session_started`,
                  metadata: this.sessionStartEvent,
                }),
                (this.sessionStartEvent = null)));
            break;
          case `thread/realtime/itemAdded`: {
            let n = Oe.safeParse(t.params.item);
            n.success &&
    ...
    handleRealtimeDataChannelMessage(e, t) {
      ...
      try {
        ...
        f.debug(`[Composer] ignored invalid realtime data channel message`, {
          safe: {},
          sensitive: {},
        });
        return;
      }
      let r = ke.safeParse(n);
      if (!r.success) return;
      let i = r.data.response_id;
      this.loggedRealtimeTurnResponseIds.has(i) ||
        (this.loggedRealtimeTurnResponseIds.add(i),
        g(e, {
          eventName: `opaline_realtime_turn_started`,
    ...
    handleSdpError(e, t) {
      ...
      ((this.startRequestId += 1),
        ...
        f.error(`[Composer] failed to accept realtime WebRTC SDP`, {
          safe: {},
          sensitive: { error: t },
        }),
        r != null &&
          T({ manager: r, conversationId: n }).catch((e) => {
            f.warning(`[Composer] failed to stop realtime after SDP error`, {
              safe: {},
              sensitive: { error: e },
            });
          }));
    ...
    handleRealtimeError(e, t) {
      ...
      ((this.startRequestId += 1),
        ...
        r != null && a(`discard-conversation-if-empty`, { conversationId: n }));
    }
    ...
    handleWebRtcConnectionFailed(e) {
      ...
      ((this.startRequestId += 1),
        ...
        n != null &&
          T({ manager: n, conversationId: t })
            .catch((e) => {
              f.warning(`[Composer] failed to stop realtime after WebRTC error`, {
                safe: {},
                sensitive: { error: e },
              });
            })
    ...
    applyRealtimeMuteState(e, t) {
      (this.session?.setOutputAudioMuted(t), e.set(X, t), this.resetEphemeralTranscript(e));
    }
    showAssistantTranscriptText(e, t) {
      ((this.assistantTranscriptText = t),
        e.get(X) && (e.set(Q, t.trim()), this.scheduleEphemeralTranscriptClear(e)));
    }
...
//# sourceMappingURL=realtime-controller-VNMyi_3-.js.map
