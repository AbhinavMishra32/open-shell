...
async function a(e) {
  try {
    ...
    t.success ||
      i.warning(`Failed to refresh Opaline runtime config from Statsig`, {
        safe: {},
...
function o(r) {
  ...
  return (
    o.success ||
      i.info(`Invalid Opaline runtime install config; using default runtime`, {
        safe: {},
...
function s(e) {
  (i.info(`Opaline runtime config selected`, {
    safe: { artifact: t, source: e.source },
    sensitive: { runtimeInstallConfig: e.value },
  }),
    r.dispatchMessage(`opaline-runtimes-config-changed`, { config: c(e.value) }));
}
function c(e) {
  return { runtimes: { [t]: e } };
}
