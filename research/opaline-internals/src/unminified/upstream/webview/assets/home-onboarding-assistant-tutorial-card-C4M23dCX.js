...
function ue({ completed: e, hostId: t, onLocalConversationCreated: n, projectRoot: r }) {
  let a = p(g),
    ...
    S = c.formatMessage({
      ...
      defaultMessage: `Setup Opaline`,
      description: `Title for the card that starts the Setup Opaline flow`,
    }),
    w = c.formatMessage({
      id: `electron.onboarding.assistantSuggestion.description`,
      defaultMessage: `Personalize Opaline around your work`,
      description: `Description for the card that starts the Setup Opaline flow`,
    }),
    T = u || r == null,
    O = async () => {
      if (!(T || e || v || r == null)) {
        (o(a, s.OPALINE_ONBOARDING_WIZARD_PHASE_LAUNCH, i.OPALINE_ONBOARDING_WIZARD_ACTION_COMPLETED),
          x(!0));
        try {
          let e = pe(h.roles);
          (await me({
            ...
            e != null &&
              o(
                a,
                s.OPALINE_ONBOARDING_WIZARD_PHASE_ROLE_PICKER,
                i.OPALINE_ONBOARDING_WIZARD_ACTION_SKIPPED,
              ),
...
function fe(e) {
  return T({ name: `Setup Opaline`, path: e });
}
function pe(e) {
  let t = e.filter((e) => e !== `default`);
  return t.length === 0
    ? null
    : `The user has already selected Setup Opaline roles: ${t.join(`, `)}. Mark Personalize Opaline complete, skip role selection, and use these roles for subsequent setup steps.`;
}
async function me({
  additionalDeveloperInstructions: e,
  agentMode: t,
