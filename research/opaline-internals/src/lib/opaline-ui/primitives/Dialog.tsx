...
export type OpalineDialogContentProps = ComponentPropsWithoutRef<typeof DialogPrimitive.Content> & {
...
>(({ className = "", ...props }, ref) => (
  <DialogPrimitive.Overlay ref={ref} className={`opaline-dialog-overlay ${className}`} {...props} />
));
DialogOverlay.displayName = "DialogOverlay";

export const DialogContent = forwardRef<ElementRef<typeof DialogPrimitive.Content>, OpalineDialogContentProps>(
  (
    {
      children,
      className = "",
    ...
    return (
      <DialogPrimitive.Portal container={portalContainer ?? undefined}>
        ...
        <DialogPrimitive.Content
          ...
          className={`opaline-dialog opaline-dialog-size-${size} ${unstyledContent ? "" : "opaline-dialog-surface"} ${contentClassName} ${className}`}
          style={style}
          onPointerDown={(event) => {
        ...
        >
          ...
          {showDialogClose ? (
            <DialogPrimitive.Close className="opaline-dialog-close" aria-label="Close">
              <CloseIcon />
...
export function DialogHeader({
  ...
  return (
    <div className={`opaline-dialog-header ${className}`}>
      {icon != null ? <span className="opaline-dialog-header-icon">{icon}</span> : null}
      <div className="opaline-dialog-header-copy">
        {title != null ? <DialogPrimitive.Title className="opaline-dialog-title">{title}</DialogPrimitive.Title> : null}
        {subtitle != null ? (
          <DialogPrimitive.Description className="opaline-dialog-description">{subtitle}</DialogPrimitive.Description>
        ) : null}
        {children}
      </div>
    </div>
...
export function DialogBody({
  ...
  const Component = as;
  return (
    <Component className={`opaline-dialog-body ${size === "full" ? "opaline-dialog-body-full" : ""} ${size === "tall" ? "opaline-dialog-body-tall" : ""} ${className}`} {...props}>
      {children}
    </Component>
  );
}
...
export function DialogFooter({ children, className = "" }: { children: ReactNode; className?: string }) {
  return <div className={`opaline-dialog-footer ${className}`}>{children}</div>;
}

export function DialogSection({ children, className = "" }: { children: ReactNode; className?: string }) {
  return <div className={`opaline-dialog-section ${className}`}>{children}</div>;
}
