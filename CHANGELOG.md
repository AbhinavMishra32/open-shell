...
- Updated `AppShell` to accept a `history` controller and drive Opaline-style sidebar chrome back/forward controls from it.
...
- Changed `@opaline/ui` to build a publishable npm package from `dist` with JS, declarations, and copied CSS assets.

## 0.2.0

- Added the generic `SlotPanel` tab system for any shell slot.
- Added xterm.js-backed `TerminalSurface` support.
- Improved Opaline-style tab and bottom-panel shapes.

## 0.1.0

- Initial Opaline UI package with `AppShell`, `Sidebar`, `Composer`, `BottomPanel`, `FileTree`, `FileBrowserPanel`, thread surfaces, and overlay primitives.
